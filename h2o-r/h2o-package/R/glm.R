...
#' @param theta_iteration_step Theta iteration step Defaults to 0.
                    ...
                    ) 
{
  ...
  if (!missing(theta))
    parms$theta <- theta
  if (!missing(theta_iteration_step))
    parms$theta_iteration_step <- theta_iteration_step
  if (!missing(solver))
    parms$solver <- solver
  if (!missing(alpha))
    parms$alpha <- alpha
